...
<div class="intro-y col-span-12 lg:col-span-12">
    ...
    <div class="intro-y box">
        <div class="flex flex-col sm:flex-row items-center p-5 border-b border-slate-200/60">
            ...
            @if ($user->level == 'admin' && $parameter->nama === 'Tingkat Implementasi Rencana Aksi RB General')
                <button class="btn btn-primary btn-sm" id="generate-rb-general">Generate Skor Otomatis</button>
            @endif
        </div>
...
<script>
    $(document).ready(function() {
        ...
        @if ($user->level == 'admin' && $parameter->nama === 'Tingkat Implementasi Rencana Aksi RB General')
        $('#generate-rb-general').on('click', function() {
            Swal.fire({
                title: 'Generate skor otomatis?',
                text: 'Skor akan diisi berdasarkan capaian output RB General.',
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('evaluasi/lke-utama/'.$parameter->id.'/generate-rb-general') }}",
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
                        success: function(res) {
                            if (res.success) {
                                Swal.fire('Berhasil', 'Skor berhasil digenerate untuk ' + res.updated + ' instansi.', 'success');
                            } else {
                                Swal.fire('Gagal', res.message || 'Tidak ada skor yang diperbaharui.', 'error');
                            }
                            getData();
                        },
                        error: function() {
                            Swal.fire('Error', 'Terjadi kesalahan saat generate skor.', 'error');
                        }
                    });
                }
            });
