    ...
    <div class="lg:col-span-12 p-5 border-b border-slate-200/60">
        <table id="perencanaan" class="table table-bordered table-striped mt-5" cellspacing="0" width="100%">
            ...
            <tbody>
                ...
                $instansi_cache = DB::table('indikator')
                    ->select('id', 'kl', 'provinsi', 'kabupaten')
                    ->get()
                    ->keyBy('id')
                    ->toArray();

                $all_plans = DB::table('general_perencanaan')
                    ->whereIn('instansi_id', $instansis->pluck('id'))
                    ->get()
                    ->groupBy('instansi_id');

                $all_targets = DB::table('general_perencanaan_target as gpt')
                    ->select(DB::raw('MIN(gpt.tahun) as tahun'), 'gp.instansi_id', DB::raw('COUNT(gpt.id) as target_count'))
                    ->join('general_perencanaan as gp', 'gp.id', '=', 'gpt.general_perencanaan_id')
                    ->whereIn('gp.instansi_id', $instansis->pluck('id'))
                ...
                $instansi_rencana_aksi = $all_rencana_aksi->get($instansi->id);

                $baseline_count = $instansi_plans->where('baseline_tahun', $tahun_berjalan - 1)->count();
                $baseline = $baseline_count >= 5 ? 'yes' : '---';

                $target_count = $instansi_target->target_count ?? 0;
                $target = $target_count >= 5 ? 'yes' : '---';

                $rencana_aksi_count = $instansi_rencana_aksi->rencana_aksi_count ?? 0;
                $rencana_aksi = $rencana_aksi_count >= 5 ? 'yes' : '---';

                $tahun = $instansi_target->tahun ?? '---';
