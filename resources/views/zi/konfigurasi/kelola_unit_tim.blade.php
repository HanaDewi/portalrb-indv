...
<div class="intro-y col-span-12 lg:col-span-12">
    ...
    <div class="intro-y box">
        <div class="flex flex-col sm:flex-row items-center p-5 border-b border-slate-200/60">
            <h2 class="font-medium text-base mr-auto"> Kelola Unit Tim</h2>
            <button class="btn btn-primary shadow-md mr-2 float-right" onclick="tambah();" data-bs-toggle="modal"
                data-bs-target="#modal-kelola-instansi"><i class="fa fa-add"></i> &nbsp; Tambah
                Instansi Tim</button>
            <button class="btn btn-danger shadow-md mr-2 float-right" onclick="tambahUnit();" data-bs-toggle="modal"
                data-bs-target="#modal-kelola-tim"><i class="fa fa-add"></i> &nbsp; Tambah Unit Tim</button>
        </div>
        <div class="lg:col-span-12 p-5 border-b border-slate-200/60">
            <table id="instansi_tim_evaluasi" class="table table-bordered table-striped table-hover" cellspacing="0"
                width="100%">
                ...
                <tbody class>
                    @foreach ($teams as $index=>$tim )
                    <tr>
                        <td>{{$index + 1}}</td>
                        <td>{{$tim->nama}}</td>
                        <td>

                            @foreach ($instansiTims[$tim->id] as $index2 => $instansiTim)
                            {{$index2 +1 }}. &nbsp;{{$instansiTim}}, &nbsp; &nbsp;
                            @endforeach
...
<div id="modal-kelola-unit-tim" class="modal fade" tabindex="-1" aria-hidden="true">
                ...
                <div class="modal-body grid columns-12 gap-4 gap-y-3">
                    <div class="g-col-12">
                        <div class="form-group">
                            <label for="nama" class="form-label">Nama Instansi <span
                                    class="text-danger">*</span></label>
                            <select class="form-control instansiId" id="instansiId" name="instansiIds[]"
                                multiple="multiple">
...
<script>
    $(document).ready(function() {
        $('.instansiId').select2({
            dropdownParent: $("#modal-kelola-unit-tim")
        });
        $('.namaTim').select2({
            dropdownParent: $("#modal-kelola-unit-tim")
        });
        getData();
        modal_kelola_anggota_tim = tailwind.Modal.getInstance(document.querySelector("#modal-kelola-unit-tim"));
        
        $('#form-kelola-unit-tim').validate({
    ...
    function hapus(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{url('/zi/kelola-unit-tim/hapus')}}",
                    type: "post",
