...
<div class="intro-y col-span-12 lg:col-span-12">
    ...
    <div class="intro-y box">
        <div class="flex flex-col sm:flex-row items-center p-5 border-b border-slate-200/60">
            <h2 class="font-bold text-base mr-auto"> Proses Sanggah - {{ auth()->user()->nama }}</h2>
        </div>
        <br />
        <div class="lg:col-span-12 p-5 border-b border-slate-200/60">
            ...
            <table id="rekap-zi" class="table table-bordered table-striped" cellspacing="0" width="100%">
                ...
                <tbody>
                    @foreach ($instansi_sanggahs as $index => $instansi_sanggah )
                    <tr>
                        <td>{{$index+1}}</td>
                        <td @if($instansi_sanggah->instansi_ZI->instansi_wbk_mandiri)
                            class="text-red-500"
                            @endif
                            >
                            {{$instansi_sanggah->instansi_ZI->klpd_instansi->name}}
                            @if($instansi_sanggah->instansi_ZI->instansi_wbk_mandiri)
                            (WBK Mandiri)
                            @else
                        ...
                        <td class="text-center">
                            ...
                            $nama_tim = [];
                            $teams =
                            $instansi_sanggah->instansi_ZI->unit_zi->flatMap->unit_tim->map->tim->unique()->pluck('nama');
                        ...
                        <td class="text-center">
                            ...
                            <!-- mandiri -->
                            @else
                            <!-- {{$instansi_sanggah->instansi_ZI->jml_wbk}} -->
                            @endif
                        </td>
                        <td class="text-center">
                            <!--{{$instansi_sanggah->instansi_ZI->jml_wbbm}}-->
                        </td>
                        <td class="text-center">
                            @if($instansi_sanggah->instansi_ZI->instansi_wbk_mandiri)
                            <!--{{$instansi_sanggah->instansi_ZI->jml_wbbm}}-->
                            @else
                            <!--{{$instansi_sanggah->instansi_ZI->jml_wbk + $instansi_sanggah->instansi_ZI->jml_wbbm}}-->
                            @endif
                        </td>
                        ...
                        <td class="text-center">
                            ...
                            <a href="{{route('proses_sanggah',$instansi_sanggah->instansi_ZI->id)}}"
