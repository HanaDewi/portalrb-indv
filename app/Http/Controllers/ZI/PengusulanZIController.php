...
use App\Helpers\ZIHelper;

class PengusulanZIController extends Controller
{
    private string $tahap = 'Pengusulan';
    ...
    public function index(Request $request)
    {
        ...
        $tahun = ZIHelper::activeYear();
        $tahap_seleksi = ZIHelper::getTahapSeleksi($tahun, $this->tahap);
        if (!$tahap_seleksi) {
            abort(404, 'Tahap seleksi belum tersedia');
        }

        if (!ZIHelper::isPeriodeStarted($tahap_seleksi)) {
            return view('zi.zibelumbuka');
        }

        $instansiObj = ZIHelper::resolveInstansi($request);
        ...
        $instansi = $instansiObj->name;
        $group_kld = $instansiObj->group;

        $instansiZI = InstansiZI::where('instansi_id', $instansiObj->id)
            ->where('tahun', $tahun)
            ->first();

        if (!$instansiZI) {
            return response(
                "Mohon maaf instansi anda belum terdapat penilaian RB di tahun lalu"
            );
        }

        if ($instansiZI->final) {
            return redirect('zi-tinjau?instansi_id=' . $instansiObj->id);
        }

        $instansis = ZIHelper::isAdminOrTpn()
            ? KlpdInstansi::orderBy('name')->get()
            : collect();
    ...
    public function store_bukti_dukung(Request $request)
    {
        $tahun = ZIHelper::activeYear();
        $tahap_seleksi = ZIHelper::getTahapSeleksi($tahun, $this->tahap);
        if (!$tahap_seleksi || !ZIHelper::isPeriodeOpen($tahap_seleksi)) {
            return back()->with(
                'error',
                'Periode pengisian belum dibuka / sudah ditutup'
            );
        }

        $instansi_id = ZIHelper::isAdminOrTpn()
            ? $request->instansi_id
            : Auth::user()->instansi_id;

        $instansiZI = InstansiZI::where('instansi_id', $instansiId)
            ->where('tahun', $tahun)
            ->firstOrFail();


        $instansiZI->tahap_seleksi = 1;
        $instansiZI->pic = $request->pic;
        $instansiZI->email = $request->email;
        $instansiZI->nomor_kontak = $request->nomor_kontak;
        $instansiZI->surat_usulan = ZIHelper::normalizeUrl($request->surat_usulan); #kalau gak ada http:// jadinya relatif link yang buka di halaman portalrb bukan di google drivenya
        $instansiZI->sptjm = ZIHelper::normalizeUrl($request->sptjm);
        $instansiZI->tlhp = ZIHelper::normalizeUrl($request->tlhp);
        $instansiZI->survei_mandiri = ZIHelper::normalizeUrl($request->survei_mandiri);
        ...
        if ($unit_wbks) {
            foreach ($unit_wbks as $key => $unit_wbk) {
                ...
                $unit_zi->lke = ZIHelper::normalizeUrl($request->lke_wbk[$key] ?? null);
        ...
        if ($unit_wbbms) {
            foreach ($unit_wbbms as $key2 => $unit_wbbm) {
                ...
                $unit_zi->lke = ZIHelper::normalizeUrl($request->lke_wbbm[$key2] ?? null);
                $unit_zi->wbbm = 1;
                $unit_zi->save();
                $wbbmCount++;
            }
