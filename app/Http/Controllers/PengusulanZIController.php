...
class PengusulanZIController extends Controller
{
    ...
    $instansi_id = Auth::User()->user_rel->instansi_id;
    ...
    foreach($lkeTestTPLine as$tpLine){
        if($tpLine->paramL4->name == "Opini BPK"){
            $skor_opini_bpk = $tpLine->score_index; #khusus BPK pakenya score index karna ada kasus menpan scorenya 1 tapi score indexnya 5 (WTP) 
        }elseif($tpLine->paramL4->name == "Nilai Sistem Akuntabilitas Kinerja Instansi Pemerintah (SAKIP)"){
            $skor_predikat_sakip = $tpLine->score;
        }elseif($tpLine->paramL4->name == "Tingkat Maturitas Sistem Pengendalian Intern Pemerintah (SPIP)"){
            $skor_maturitas_spip = $tpLine->score;
        }
    }
    ...
    #===============Syarat TIngkat Instansi Pusat
    #syarat BPK
    if($opini_bpk == "WTP"){
        $syarat_bpk = "LULUS";
    }
    #Syarat SAKIP
    if($skor_predikat_sakip > 60){ #predikat B keatas
        $syarat_sakip_wbk = "LULUS";
    }
    if($skor_predikat_sakip > 70){ #predikat B keatas
        $syarat_sakip_wbbm = "LULUS";
    }
    #Syarat Indeks RB
    if( $group_kld == "prov" || $group_kld == "kab" ){#Jika Pemda
        if(ceil($skor_indeks_rb) > 50){ #index CC keatas
            $syarat_indeksrb_wbk = "LULUS";
        }
        if(ceil($skor_indeks_rb) > 60){
            $syarat_indeksrb_wbbm = "LULUS";
        }
    }elseif($group_kld  == "kl"){#Jika KL
        if(ceil($skor_indeks_rb) > 60){ #index CC keatas
            $syarat_indeksrb_wbk = "LULUS";
        }
        if(ceil($skor_indeks_rb) > 70){
            $syarat_indeksrb_wbbm = "LULUS";
        }
    }
    #Syarat SPIP
    if(ceil($skor_maturitas_spip) >= 3){ #predikat B keatas
        $syarat_maturitas_spip = "LULUS";
    }
    ...
    public function generate_rekap_instansi_skor(){
        ...
        foreach($instansis as $instansi){
            ...
            $lkeTestTPLine =  $lkeTestTP->lke_test_tp_line;
            $skor_indeks_rb = $lkeTestTP->index_rb;
            $skor_opini_bpk ="";
            $skor_predikat_sakip = "" ;
            $skor_maturitas_spip ="";
            ...
            $instansiZI = new InstansiZI();
            $instansiZI->instansi_id = $instansi_id;
            $instansiZI->skor_bpk = $skor_bpk;
            ...
            $instansiZI->syarat_indeksrb_wbkbm= $syarat_indeksrb_wbbm;
